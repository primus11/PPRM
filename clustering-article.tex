...
\author{Matej Pičulin, Primož Bajželj, Matej Drame}
...
One of the techniques used is clustering. In this paper a few of the most known clustering
algorithms are researched and compared to each other.
\end{abstract}

\begin{IEEEkeywords}
clustering, clustering validation, clustering algorithms
\end{IEEEkeywords}
...
but don't include clustering based on information theory or other progress of the last decade.
...
parameter. Hierarchical clustering algorithms essentially output clusterings (a clustering
...
The average silhouette of the data is a useful criterion for assessing the number of clusters in the data.
...
The number of clusters should be chosen so that adding another cluster doesn't give much better modeling
...
There are two common ways of selecting the initial centroids: random seed and random partition.
...
therefore actually lowers the overall computation time.
...
The algorithm takes examples from a data stream and proceeds with one of three actions: updating the corresponding cluster radius, changing
the cluster center or creating a new cluster.
...
of the dataset. Examples in sparse areas between clusters are considered noise. The notion
of noise actually particularly sets these algorithms apart from the others.
...
\section{Results}
To experimentally compare algorithms and see whether or not we can confirm
their theoretical properties, their advantages and disadvantages, we set up a test.
We used \textit{scikit-learn} machine learning library written in Python \cite{scikit}
to test k-means, GMM and DBSCAN. We used our own implementation of ECMC and an \textit{R} library
called \textit{kernlab} \cite{kernlab} provided an implementation of Spectral clustering.


\subsection{Datasets}

For each algorithm comparison on three different artificial datasets was done. The first dataset, shown in figure \ref{dataset1},
is comprised of two linearly separable clusters. The second one, shown in figure \ref{dataset2}, is not linearly separable
which makes it difficult for some algorithms to cluster it correctly. In this second dataset one
whole cluster is inside another cluster. The last dataset's shape resembles two half-moons. This dataset is
not linearly separable either. Presented on figure \ref{dataset3}. Different colors represent different clusters.
Note that figures \ref{dataset1}, \ref{dataset2} and \ref{dataset3} display the desired clustering result.
...
This was a simple problem and all algorithms were successful as we can see in table \ref{redblueresults}.
DBSCAN performed slightly worse than others. The reason is that DBSCAN is detecting noise as well.
...
This dataset is actually the most difficult one in our comparison. ECMC, GMM and k-means
fail to correctly cluster the data. On the other hand, this dataset is easy for DBSCAN and spectral clustering.
We can see the results in table \ref{circleweirdresults}.
%the outcome are bad results for most of
...
This dataset also turned out to be difficult, but algorithms still achieved better scores than with ''nested circle'' dataset.
DBSCAN and spectral clustering again had no problem (see in table \ref{halfmoonsresults}). However, because of
DBSCAN's noise detection spectral clustering did better.
...
\caption{DBSCAN on half-moons. Notice the examples marked as noise (green squares).}
...
Clustering is a broad topic with many different subproblems and approaches of solving them.
No single clustering algorithm satisfies all possible requirements data analysts might have.
We can provide some guidelines though.

Spectral clustering did particularly well in our tests, followed closely by
DBSCAN which has the advantage (or disadvantage, depending on the problem) of detecting noise in data.
Other algorithms did well in simple cases but not in more complex ones.

However, spectral clustering was considerably slower (at least the implementation we used was)
than other approaches. k-means was the fastest (should not be surprising). Results of ECMC
were similar to k-means. We didn't use a dataset that would show where GMM excels.
...
The authors would like to thank the mentor for his help and suggestions.
